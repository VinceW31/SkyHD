...
from RM3settings import *
import re
from os import path
from Crypto.Cipher import AES

#SettingsFile = ConfigParser.ConfigParser()
#SettingsFile = configparser.ConfigParser() #Python3
#SettingsFile.optionxform = str
#SettingsFile.read(RM3settings.BlackBeanControlSettings)
...
##            return 2
    if power != '':
        DecodedCommand_power = binascii.unhexlify(power)
    if mute != '':
        DecodedCommand_mute = binascii.unhexlify(mute)
    if volup != '':
        DecodedCommand_volup = binascii.unhexlify(volup)
    if voldown != '':
        DecodedCommand_voldown = binascii.unhexlify(voldown)
    
    if SentCommand == power:
        if power != '':
            DecodedCommand_power = binascii.unhexlify(power)
            RM3Device.send_data(DecodedCommand_power)
        else:
            RM3Device.enter_learning()
            time.sleep(RealTimeout)
            power = RM3Device.check_data()
            if power is None:
                print('Command not received')
                sys.exit()
                
            print(power)
            Encoded_power_Command = binascii.hexlify(power).decode("ascii")
            ...
            if Encoded_power_Command:
                ...
                #SettingsFile.write(BlackBeanControlIniFile)
                File.close()
                print('Set command {0}'.format(power))
                
    elif SentCommand == mute:
        if mute != '':
            DecodedCommand_mute = binascii.unhexlify(mute)
            RM3Device.send_data(DecodedCommand_mute)
        else:
            RM3Device.enter_learning()
            time.sleep(RealTimeout)
            mute = RM3Device.check_data()
            if mute is None:
                print('Command not received')
                sys.exit()
                
            print(mute)
            Encoded_mute_Command = binascii.hexlify(mute).decode("ascii")
            ...
            if Encoded_mute_Command:
                ...
                #SettingsFile.write(BlackBeanControlIniFile)
                File.close()
                print('Set command {0}'.format(mute))
        
    elif SentCommand == volup:
        if volup != '':
            DecodedCommand_volup = binascii.unhexlify(volup)
            RM3Device.send_data(DecodedCommand_volup)
        else:
            RM3Device.enter_learning()
            time.sleep(RealTimeout)
            volup = RM3Device.check_data()
            if volup is None:
                print('Command not received')
                sys.exit()
                
            print(volup)
            Encoded_volup_Command = binascii.hexlify(volup).decode("ascii")
            ...
            if Encoded_volup_Command:
                ...
                #SettingsFile.write(BlackBeanControlIniFile)
                File.close()
                print('Set command {0}'.format(volup))
        
    elif SentCommand == voldown:
        if voldown != '':
            DecodedCommand_voldown = binascii.unhexlify(voldown)
            RM3Device.send_data(DecodedCommand_voldown)
        else:
            RM3Device.enter_learning()
            time.sleep(RealTimeout)
            voldown = RM3Device.check_data()
            if voldown is None:
                print('Command not received')
                sys.exit()
                
            print(voldown)
            Encoded_voldown_Command = binascii.hexlify(voldown).decode("ascii")
            ...
            if Encoded_voldown_Command:
                ...
                #SettingsFile.write(BlackBeanControlIniFile)
                File.close()
                print('Set command {0}'.format(voldown))
...
##        if EncodedCommand:
##            File = open('RM3settings.py'), 'w')
##            File.append(SentCommand, EncodedCommand)
            #SettingsFile.set('Commands', SentCommand, EncodedCommand)
            #SettingsFile.write(BlackBeanControlIniFile)
##            File.close()
...
if __name__ == "__main__":
    ...
    try:
        ...
        print('BlackBeanControl2.py -c <Command name> [-d <Device name>] [-i <IP Address>] [-p <Port>] [-m <MAC Address>] [-t <Timeout>] [-r <Re-Key Command>]')
        sys.exit(2)
